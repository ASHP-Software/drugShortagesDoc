...
Note: If you are testing the data on our staging database, we do not keep that dataset up to date like we do in our production database, which is updated with new content daily. You can see an user interface of our production data [here](https://www.ashp.org/shortages?loginreturnUrl=SSOCheckOnly). The purpose of the staging database is to give you an understanding of how we structure our data. You can also review a FAQ of our drug information content [here](https://www.ashp.org/Drug-Shortages/Current-Shortages/Drug-Shortages-FAQs).
...
A product will go from affectedProduct list to the availableProduct list accross drug shortage bulletin versions. It is also possible to go from the availableProduct list back to the affectedProduct list if the product is in shortage again over time.   
